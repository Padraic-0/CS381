...
sub build_bigrams {
	...
	for <a an and by for from in of on or out the to with> -> $word {
    	%counts{$word}:delete;
	}
	#say %counts;
...
sub mcw {
	...
	##########################
	my @best_words =%counts{$word}.keys.sort({ %counts{$word}{$_} }).reverse;#%counts{$word}.keys.sort({-%counts{$word}{$_}});
	#print("#############################\n");
	#print($word, "\n");
	#print(@best_words);
	#print("\n#############################");
...
sub sequence {
	my $word = @_[0];
	if ($DEBUG) {say "<sequence for \'$word\'>\n"};	
	...
	my $title = "";
	$title ~= $word;
	for 2..$SEQUENCE_LENGTH -> $i {
		my $new_word = mcw($word);
		if $new_word eq '' {
			last;
		} else {
			$title ~= " " ~ $new_word ;
